...
usersCollection.load().then(() => {}).catch((err) => {
  logger.error(err)
})

export const fagotDetect = () => {
  appInteraction.reply('Кручу верчу пидора найти хачу!').then(() => {
    findFag('')
  }).catch(err => {
    logger.error(err)
  })
...
export const botInfo = () => {
  appInteraction.reply(BotInfoMsg).then(() => {
  }).catch(err => {
    logger.error(err)
  })
}
...
export const findFag = (argMsg = '') => {
  client.channels.fetch(process.env.CHANNEL_ID).then(async channel => {
    ...
    const users = await getUsers({ createdAt: today }).then(res => {
      return res
    }).catch(err => {
      logger.error(err)
    })
    ...
    if (users.length === 0) {
      ...
      createUser({ ...randomUser, createdAt: today }).then(async () => {
...
const getRandomGif = async () => {
  const genres = ['bonk', 'blush', 'smile', 'slap', 'kick', 'happy', 'wink', 'poke', 'dance']
